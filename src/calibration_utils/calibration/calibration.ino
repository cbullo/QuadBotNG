...
void testAlignmentAndCogging(int direction) {
  ...
  for (int i = 0; i < sample_count; i++) {
    ...
    // measure 
    float sensorAngle = motor.sensor_direction * (sensor.getAngle() - initialAngle) * 180.f / PI;
...
void setup() {
  ...
  float mean_zero_offset = 0.f;
  for (int i = 0; i < 10; ++i) {
    motor.zero_electric_angle = NOT_SET;
    motor.initFOC();
    mean_zero_offset += 0.1f * motor.zero_electric_angle;
  }
